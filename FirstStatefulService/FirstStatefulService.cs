...
namespace FirstStatefulService
{
    ...
    /// </summary>
    internal sealed class FirstStatefulService : StatefulService, ICounter
    {
        private static string theCountsDictionaryName = "myDictionary";
        private static string theCountsKey = "Counter-1";

        public async Task<long> GetCountAsync()
        {
            var myDictionary = await this.StateManager.GetOrAddAsync<IReliableDictionary<string, long>>(theCountsDictionaryName);
            using (var tx = this.StateManager.CreateTransaction())
            {
                var result = await myDictionary.TryGetValueAsync(tx, theCountsKey);
        ...
        protected override async Task RunAsync(CancellationToken cancelServicePartitionReplica)
        {
            ...
            while (!cancelServicePartitionReplica.IsCancellationRequested)
            {
                ...
                using (var tx = this.StateManager.CreateTransaction())
                {
                    ...
                    await myDictionary.AddOrUpdateAsync(tx, theCountsKey, 0, (k, v) => ++v);
